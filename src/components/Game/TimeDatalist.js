...
function TimeDatalist({ hltbInfo, setValue }) {
	...
	return (
		<div className='timeDatalist' hidden={!hltbInfo}>
			<div
				className='timeData'
				hidden={!hltbInfo || hltbInfo === "0 часов" || hltbInfo?.gameplay_main === -1}
				onClick={() => setValue(strToFloat(hltbInfo?.gameplay_main ? hltbInfo?.gameplay_main : hltbInfo))}>
				<MDBIcon far icon='clock' className='light-green-text' />
				{strToFloat(hltbInfo?.gameplay_main ? hltbInfo?.gameplay_main : hltbInfo)}
			</div>
			<div className='timeData' hidden={hltbInfo?.gameplay_main_extra === -1} onClick={() => setValue(strToFloat(hltbInfo?.gameplay_main_extra))}>
				<MDBIcon far icon='clock' className='yellow-text' />
				{strToFloat(hltbInfo?.gameplay_main_extra)}
			</div>
			<div className='timeData' hidden={hltbInfo?.gameplay_completionist === -1} onClick={() => setValue(strToFloat(hltbInfo?.gameplay_completionist))}>
